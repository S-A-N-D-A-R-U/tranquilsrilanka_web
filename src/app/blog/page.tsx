    ...
    <>
      ...
      <section className="container-page section-y">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {posts.map((p: any) => (
            <Link key={p.slug} href={getPostLink(p)} target={getPostTarget(p)} className="group card-surface overflow-hidden hover:shadow-elegant transition flex flex-col">
              ...
              <div className="p-6 flex flex-col flex-grow">
                <div className="flex items-center gap-2 text-xs text-muted-foreground"><Calendar className="h-3.5 w-3.5" /> {new Date(p.createdAt).toLocaleDateString()} · {p.readTime}</div>
